...
client.on("guildMemberAdd", member => {  
    ...
    let alaycı = client.users.cache.get(member.id);
...
'9': (ayarlar.sayı9),}[d];
 ...
 const hgmesaj = (`hoşgeldin (<@${alaycı.id}>) \n\n Seninle beraber sunucumuz(`  + üyesayısı +  `)kişiye ulaştı. \n\n Hesabın (`  + moment(member.user.createdAt).format("DD MMMM YYYY dddd") + `) oluşturulmuş`  + kontrol + `\n\n Kaydını tamamlamak için herhangi bir \`V.Confirmation\` ses teyit odasına girmen yeterlidir. \n\n Kayıt olduktan sonra rol-seçim katagorisinden rolleri almayı unutma. \n\n Tagımızı alarak ailemizin bir parçası olabilirsin. \`TAGINNEYSEONUKOY\` <@&KAYITÇIROLID> `)
kanal.send(hgmesaj)
...
client.on("guildMemberAdd", member => {
  ...
  member.user.send('Selam Nasılsın Umarım İyisindir Malesef Hesaın 7 Günden Önce Oluşturulduğu İçin Şüpheli Sayılıyorsun 7 Gün Dolduktan Sonra Yetkilerle İletişime Geç Onlar Sana Yardım Edecekler')
  setTimeout(() => {
